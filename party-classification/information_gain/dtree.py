...
class dtree:
    ...
    # def modify_classes(self, data, test):
    #     for i in range(len(test)):
    #         self.classes.append(test[i][-1])

    def read_data(self, filename):

        csv_data = pd.read_csv("house-votes-84.data", header=None)
        csv_data.replace({"?": np.nan}, inplace=True)
        for i in range(1, len(csv_data.columns)):
            csv_data[i].fillna(csv_data[i].mode()[0], inplace=True)
        csv_data.to_csv("processed_data.data", header=None, index=False)
        ...
        for i in range(len(train)):
            self.classes.append(train[i][0])
            train[i] = train[i][1:]

        test_classes = []

        for i in range(len(test)):
            test_classes.append(test[i][0])
            test[i] = test[i][1:]

        return train, test, self.classes, test_classes, self.featureNames, dataset_length
